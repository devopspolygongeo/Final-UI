...
export class PlotviewGalleryComponent implements OnInit {
  ...
  private async loadAssets(surveyId: number) {
    try {
      const assets: AssetRow[] = await firstValueFrom(
        this.assetsApi.getAssets(surveyId),
      );
      ...
      (assets ?? []).forEach((a: AssetRow) => {
        const typeValue = (a?.type || '').toString().toLowerCase();

        const isImage = typeValue === 'image' || a?.asset_type === 1;
        const isVideo = typeValue === 'video' || a?.asset_type === 2;

        // gallery must only show images and videos
    ...
    try {
      for (const file of files) {
        if (file.size > this.maxFileSize) {
          alert(`"${file.name}" is larger than 1GB.`);
          continue;
        }
        const isImage = file.type?.startsWith('image/');
        const isVideo = file.type?.startsWith('video/');

        if (this.activeTab === 'images' && !isImage) {
          alert(`"${file.name}" is not an image.`);
          continue;
        }
        if (this.activeTab === 'videos' && !isVideo) {
