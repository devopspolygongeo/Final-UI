...
async loadLayoutDetails() {
  try {
    ...
    // ✅ PLOT ATTRIBUTES
    // ===============================
    const selectedAttrs = res.hoverAttributes
      ? res.hoverAttributes.split(',')
      : [];

    this.availablePlotAttributes.forEach(attr => {
      attr.selected = selectedAttrs.includes(attr.label);
    });
