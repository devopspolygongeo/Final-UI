...
export class PlotviewDocumentsComponent implements OnInit {
  ...
  safeSelectedDocumentUrl: SafeResourceUrl | null = null;
  isModalActive = false;
  ...
  private async loadDocuments(surveyId: number) {
    try {
      ...
      (assets ?? []).forEach((a: AssetRow) => {
        ...
        const isDoc = a?.type === 'document' || a?.asset_type === 3;
        ...
        const uploaded = (a as any).uploadDate
          ? new Date((a as any).uploadDate)
          : new Date();
  ...
  private async handleFiles(files: File[]) {
    ...
    try {
      ...
      if (this.surveyId) await this.loadDocuments(this.surveyId);
    }
  }
  ...
  openDocumentModal(document: DocumentItem) {
    ...
    this.safeSelectedDocumentUrl = this.canEmbedDocument(document.type)
      ? this.sanitizer.bypassSecurityTrustResourceUrl(document.url)
      : null;
  ...
  closeModal() {
    ...
    this.safeSelectedDocumentUrl = null;
  ...
  viewDocument(document: DocumentItem) {
    if (document.url.startsWith('blob:')) {
      ...
      if (w) w.location.href = document.url;
    } else {
  ...
  isPdfDocument(m: string) {
    return m === 'application/pdf';
  }

  isWordDocument(m: string) {
    return (
