...
export class PlotviewDashboardComponent implements OnInit {
  ...
  dashboardData: DashboardData = {
    account: { name: 'Speed', organization: 'Speed PVT LTD' },
    subscription: {
      type: 'PV001',
      asset: 'Harmony Meadows',
      validTill: '145 days',
      daysRemaining: 145,
      validDate: '12th June 2024',
    },
    ...
    this.assetService.getAllSurveys().subscribe({
      next: (surveys: any[]) => {
        const firstSurvey = surveys.find(
          (s) => Number(s.projectId) === asset.projectId,
        );
  ...
  get filteredAssets() {
    return this.dashboardData.assets.filter((asset) => {
      ...
      if (this.activeTab === 'Ongoing Assets')
        return matchesSearch && asset.status === 'Active';
      if (this.activeTab === 'Closed Assets')
        return matchesSearch && asset.status === 'Inactive';
  ...
  onDeleteAsset(asset: Asset): void {
    ...
    this.dashboardData.assets = this.dashboardData.assets.filter(
