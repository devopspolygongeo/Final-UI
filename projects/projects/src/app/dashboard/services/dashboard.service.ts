...
export class DashboardService {
  ...
  public getView(): Observable<View> {
    if (!this.view$) {
      this.view$ = this.http
        .get<View>(environment.apiUrl + '/view')
        .pipe(shareReplay(1));
    }
    return this.view$;
  }

  // ✅ Share meta resolver (added)
  public getShareMeta(token: string): Observable<any> {
    return this.http.get<any>(`${environment.apiUrl}/share/${token}/meta`, {
      headers: { 'No-Auth': 'True' },
    });
  }

  // ✅ NEW: fetch full project (added, does not affect existing flows)
  ...
  // ✅ NEW: fetch full survey (added, does not affect existing flows)
  public getSurveyById(surveyId: number): Observable<Survey> {
    return this.http.get<Survey>(`${environment.apiUrl}/surveys/${surveyId}`);
  }
  ...
  public getProjects(userId: number): Observable<Project[]> {
    if (!this.projectsMap$ || !this.projectsMap$.has(userId)) {
      this.projectsMap$.set(
        userId,
        this.http
          .get<Project[]>(environment.apiUrl + '/projects?userId=' + userId)
          .pipe(shareReplay(1)),
      );
    }
  ...
  public getSurveys(projectId: number): Observable<Survey[]> {
    if (!this.surveysMap$ || !this.surveysMap$.has(projectId)) {
      this.surveysMap$.set(
        projectId,
        this.http
          .get<Survey[]>(environment.apiUrl + '/surveys?projectId=' + projectId)
          .pipe(shareReplay(1)),
      );
    }
  ...
  public getSources(surveyId: number): Observable<Source[]> {
    if (!this.sourcesMap$ || !this.sourcesMap$.has(surveyId)) {
      this.sourcesMap$.set(
        surveyId,
        this.http
          .get<Source[]>(environment.apiUrl + '/sources?surveyId=' + surveyId)
          .pipe(shareReplay(1)),
      );
    }
    ...
    if (!this.layersMap$ || !this.layersMap$.has(sourceIds)) {
      this.layersMap$.set(
        ...
        this.http
          .get<
            Layer[]
          >(environment.apiUrl + '/vectorLayers?sourceId=' + sourceIds)
          .pipe(shareReplay(1)),
      );
    }
  ...
  public getGroups(surveyId: number): Observable<Group[]> {
    if (!this.groupsMap$ || !this.groupsMap$.has(surveyId)) {
      this.groupsMap$.set(
        surveyId,
        this.http
          .get<Group[]>(environment.apiUrl + '/groups?surveyId=' + surveyId)
          .pipe(shareReplay(1)),
      );
    }
  ...
  public getLayouts(surveyId: number): Observable<Layout[]> {
    if (!this.layoutsMap$ || !this.layoutsMap$.has(surveyId)) {
      this.layoutsMap$.set(
        surveyId,
        this.http
          .get<Layout[]>(environment.apiUrl + '/layouts?surveyId=' + surveyId)
          .pipe(shareReplay(1)),
      );
    }
  ...
  public getAssets(surveyId: number): Observable<Asset[]> {
    if (!this.assetsMap$ || !this.assetsMap$.has(surveyId)) {
      this.assetsMap$.set(
        surveyId,
        this.http
          .get<Asset[]>(environment.apiUrl + '/assets?surveyId=' + surveyId)
          .pipe(shareReplay(1)),
      );
    }
