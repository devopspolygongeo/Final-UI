...
export class DashboardService {
  ...
  public getSources(surveyId: number): Observable<Source[]> {
    console.log('Fetching sources for surveyId: from dashboard.service.ts', surveyId);
  ...
  public getPlots(surveyId: number): Observable<PlotDetails[]> {
    if (!this.plotsMap$ || !this.plotsMap$.has(surveyId)) {
      this.plotsMap$.set(surveyId, this.http.get<PlotDetails[]>(environment.apiUrl + '/plots?surveyId=' + surveyId).pipe(shareReplay(1)));
    }
    return this.plotsMap$.get(surveyId) || of([]);
  }

  public getCachedView(): Observable<View> {
    return this.view$;
  }

  public getSurveyByUserId(userId: number): Observable<Survey[]> {
    if (!this.surveysMap$ || !this.surveysMap$.has(userId)) {
      this.surveysMap$.set(userId, this.http.get<Survey[]>(environment.apiUrl + '/surveys?userId=' + userId).pipe(shareReplay(1)));
    }
    return this.surveysMap$.get(userId) || of([]);
  }
