...
export class MapComponent implements OnInit {
  ...
  private toggleVisibility(toggleItems: Toggle[]) {
    toggleItems.forEach(toggleItem => {
      this.map.setLayoutProperty(toggleItem.id, "visibility", toggleItem.checked ? 'visible' : 'none');
    });
    // if the toggles are of filter classify group type, then set filters on the map
    const isFilterToggle = toggleItems.some(toggleItem => toggleItem.metaData?.groupType && toggleItem.metaData.groupType === AppConstants.CLASSIFY_BY_FILTER);
    if (isFilterToggle) {
